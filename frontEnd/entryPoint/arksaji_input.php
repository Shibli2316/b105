...
if (isset($_GET['delete'])) {
    $_id = $_GET['delete'];
    $delete = true;
    $sql = "DELETE FROM `arksaji` where `_id`=$_id ";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['_idEdit'])) {
        ...
        $sql = "UPDATE `arksaji` SET `name`= '$name' , `unit_size`= '$unitSize', `no_emp` = '$noe', `no_days` = '$nod', `emp_days` = '$eWork', `e_sal_day` = '$eSal', `cost` = '$cost' WHERE `arksaji`.`_id`=$_id";
        $result = mysqli_query($conn, $sql);
        if ($result) {
            $update = true;
        } else {
        ...
        // SQL query to be executed
        // INSERT INTO `arksaji` (`_id`, `name`, `unit_size`, `no_emp`, `no_days`, `emp_days`, `e_sal_day`, `cost`) VALUES (NULL, 'test', '1', '1', '1', '1', '1', '1');

        $sql = "INSERT INTO `arksaji` (`name`, `unit_size`, `no_emp`, `no_days`, `emp_days`, `e_sal_day`, `cost`) VALUES ('$name', '$unitSize', '$noe', '$nod', '$eWork', '$eSal', '$cost')";
        $result = mysqli_query($conn, $sql);
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="arksaji_input.php" method="post">
    ...
    Array.from(deletes).forEach((element) => {
        element.addEventListener("click", (e) => {
            ...
            if (confirm("Are you sure")) {
                console.log("yes");
                window.location = `arksaji_input.php?delete=${_id}`;
            } else {
                console.log("no");
            }
        })
